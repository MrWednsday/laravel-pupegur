<template>
    <div>
        <div class="raw">
            <span class="col-lg-1 col-md-1 col-sm-1 col-xs-1 mr-1" v-for="tag in tags" :key="tag">
                <button class="tag" v-on:click="removeTag(tag)">{{tag}}
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-clipboard-minus" viewBox="0 0 16 16">
                        <path fill-rule="evenodd" d="M4 1.5H3a2 2 0 0 0-2 2V14a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V3.5a2 2 0 0 0-2-2h-1v1h1a1 1 0 0 1 1 1V14a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1V3.5a1 1 0 0 1 1-1h1v-1z"/>
                        <path fill-rule="evenodd" d="M9.5 1h-3a.5.5 0 0 0-.5.5v1a.5.5 0 0 0 .5.5h3a.5.5 0 0 0 .5-.5v-1a.5.5 0 0 0-.5-.5zm-3-1A1.5 1.5 0 0 0 5 1.5v1A1.5 1.5 0 0 0 6.5 4h3A1.5 1.5 0 0 0 11 2.5v-1A1.5 1.5 0 0 0 9.5 0h-3zm-1 9.5A.5.5 0 0 1 6 9h4a.5.5 0 0 1 0 1H6a.5.5 0 0 1-.5-.5z"/>
                    </svg>
                </button>
            </span>
        </div>
        <div>
            <textarea-auto-size
                :data="{ 
                        text: this.tagString,
                        enable: true,
                        inputHeight: 0,
                        error: '',
                        placeholder: 'Enter post tags here'
                    }"
                v-on:input="updateTagString"
            />
        </div>
    </div>
</template>

<script>
    export default {
        data: function(){
            return {
                tags: [],
                tagString: '',

            }},
        methods: {
            updateTagString: function(data) {
                this.tagString = data.text;
            },
            removeTag: function(tag) {
                this.tags.splice(this.tags.indexOf(tag), 1);
            }
        },

        watch: {
            tagString () {
                if(this.tagString.length !== 0){
                    const splitArr = this.tagString.split("")
                    if(splitArr.slice(-1)[0] === "\n" || splitArr.slice(-1)[0] === " "){
                        if(splitArr.length > 0){
                            this.tags.push(this.tagString.slice(0, -1));
                            this.tagString = '';
                            this.$emit('addTag', this.tags);
                        }
                    }
                }
            },
        },
    }
</script>